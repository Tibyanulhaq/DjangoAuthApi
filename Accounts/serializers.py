...
class UserPasswordSendEmailSerializer(serializers.Serializer):
    email=serializers.EmailField(max_length=255)
    class Meta:
        fields=['email']
...
class ResetPasswordSerializer(serializers.Serializer):
    ...
    def validate(self,attrs):
        try:
            ...
            if password!=password2:
                raise serializers.ValidationError({'error':'Password and confirmed password does not matched!'})
                
            id=smart_str(urlsafe_base64_decode(uid))
            user=User.objects.get(id=id)

            if not PasswordResetTokenGenerator().check_token(user,token):
                raise serializers.ValidationError({'error':'token is invalid or expired!'})
